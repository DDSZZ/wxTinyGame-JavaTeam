#前端
为组件挂载脚本，调用另一个服务脚本的静态方法；在服务脚本中进行前端数据处理，或调用后端方法；后端对数据进行处理，并访问数据库
## Scene
+ **启动窗口**
...
+ **Person**
	+ **属性**
		* INFECTED_RADIUS `static` `double` 感染半径，在每个未带口罩的病人周围感染别人
		* x `double` 在地图中的横坐标
		* y `double` 在地图中的纵坐标
		* isWearingMask `boolean` 是否戴着口罩
		* isPatient `boolean` 是否是病人
		* isWillLeave `boolean` 是否是将要离开地图的行人
	+ **方法**
		* 此行人会在**区域**内乱走(或走或停止)
		* 如果isWillLeave为true，此行人会选择一个方向并离开**区域**
		* 患病且未戴口罩的感染**感染半径**内的未戴口罩的行人
